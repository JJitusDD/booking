...
import (
	...
	"microservice-template-ddd/internal/booking/application"
...
func InitializeBookService(ctx context.Context) (*BookService, func(), error) {
	...
	service, err := NewBookApplication(bookStore, userRPCClient)
	if err != nil {
		cleanup3()
		cleanup2()
		cleanup()
...
type BookService struct {
	...
	bookRPCServer *booking_rpc.BookingServer
}
...
func NewBookApplication(store2 *store.BookStore, userRPC user_rpc.UserRPCClient) (*book.Service, error) {
	bookService, err := book.New(store2, userRPC)
	if err != nil {
		return nil, err
	}
...
func InitTracer(ctx context.Context, log *zap.Logger) (opentracing.Tracer, func(), error) {
	viper.SetDefault("TRACER_SERVICE_NAME", "microservice-template-ddd")
